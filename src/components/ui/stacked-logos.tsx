...
export const StackedLogos = ({
  ...
  return (
      ...
      >
          <div ref={gridRef} className="grid relative mx-auto w-max" style={{ gridTemplateColumns: `repeat(${columns}, ${logoWidth})` }}>
              {/* Background glow */}
              ...
              {logoGroups.map((logos, groupIndex) => (
                  <div key={groupIndex} className="stacked-logos__cell relative grid"
                      ...
                      {groupIndex === 0 && <div className="absolute top-0 bottom-0 left-0 w-px bg-white/5" />}
                      ...
                      {logos.map((logo, logoIndex) => (
                          <div key={logoIndex} className="stacked-logos__item col-start-1 row-start-1 grid place-items-center py-16 px-8"
                              ...
                              <div className="stacked-logos__logo w-full h-8 flex items-center justify-center [&>svg]:h-full [&>svg]:w-auto [&>svg]:text-white/60">
                                  {logo}
                              </div>
                          </div>
                      ))}
