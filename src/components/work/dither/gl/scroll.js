// Wheel, drag and touch all funnel into one number. `current` chases `target`
// with a lerp for the inertia feel.
//
// PORTED: upstream this ran as a fullscreen page that never scrolled - it
// called preventDefault() on wheel and set touchAction:'none', swallowing the
// gesture entirely. On this site the section lives in a normal Lenis-driven
// page and must hand off to the next section when it ends, so a second mode
// was added:
//
//   external: false  - upstream behaviour, the canvas owns the wheel
//   external: true   - the canvas listens to nothing; the host drives
//                      setProgress() from a pinned ScrollTrigger
//
// In external mode the snap spring and the click tween are both disabled.
// ScrollTrigger is already the authority on position, and a spring pulling
// toward the nearest slot would fight the scrub every frame.
...
export function createScrollController(element, config, options = {}) {
  ...
  // In external mode none of this is attached. Leaving touchAction alone is
  // the important half: setting it to 'none' kills vertical page scrolling on
  // touch devices for as long as the finger is over the canvas, which for a
  // full-viewport pinned section means the page cannot be scrolled at all.
  if (!external) {
  ...
  return {
    state,
    external,
    // Host-driven position, in the same slot units as `current`. Written every
    // frame by the ScrollTrigger scrub; the lerp in update() is what keeps it
    // from reading as a hard follow.
    setProgress(value) {
      state.target = value;
    },
    ...
    goTo(value, options = {}) {
      // Ignored under external control: the scrub would overwrite `target` on
      // the very next frame, so the tween would stutter and lose.
      if (external) return;
      tween = {
        from: state.current,
        to: value,
    ...
    update() {
      ...
      // External: no snap, no tween, no autoSpin fight. Just ease toward
      // whatever the host last wrote and keep the derived velocities, which
      // the motion bend and the streak blur both still read.
      if (external) {
